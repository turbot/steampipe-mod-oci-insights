...
query "oci_block_storage_boot_volume_name_for_volume" {
  sql = <<-EOQ
    select
      display_name as "Boot Volume"
    from
      oci_core_boot_volume
    where
...
query "oci_block_storage_boot_volume_backup_for_volume" {
  sql = <<-EOQ
    select
...
dashboard "oci_block_storage_boot_volume_detail" {
  ...
  container {
    ...
    card {
      ...
      query = query.oci_block_storage_boot_volume_faulty_for_volume
      args = {
        id = self.input.volume_id.value
      }
    }
    ...
    card {
      query = query.oci_block_storage_boot_volume_backup_for_volume
      width = 2
