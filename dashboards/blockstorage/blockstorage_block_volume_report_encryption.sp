dashboard "block_storage_block_volume_encryption_report" {
  ...
  container {
    ...
    card {
      sql   = query.block_storage_block_volume_default_encrypted_volumes_count.sql
      width = 2
    }

    card {
      sql   = query.block_storage_block_volume_customer_managed_encryption_count.sql
      width = 2
    }
  ...
  table {
    ...
    sql = query.block_storage_block_volume_encryption_report.sql
  }

}

query "block_storage_block_volume_encryption_report" {
  sql = <<-EOQ
      select
